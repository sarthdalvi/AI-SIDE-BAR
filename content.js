/* content.js - Fixed Dragging, Memory & Context Menu */
...
if (!document.getElementById("ai-float-btn")) {
  ...
  // 3. Simple Context Menu (Hidden by default)
  ...
  chrome.storage.local.get(["robotPos"], (result) => {
    if (result.robotPos) {
      ...
      // Ensure it stays visible even if window size changes
  ...
  /* --- 4. DRAG LOGIC (The Missing Part) --- */
  let isDragging = false;
  let startX, startY, initialLeft, initialTop;
  let dragThreshold = 0; // Distinguish click vs drag
  ...
  btn.addEventListener("mousedown", (e) => {
    ...
    // Disable transition for instant following
  ...
  // Toggle Sidebar (Only if NOT dragged)
  ...
  // Show "Explain" on Text Selection
  ...
  // Listen for "Close" from Sidebar Back Button
